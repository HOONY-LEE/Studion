...
- **빨강 계열을 쓰지 않는다.** accent color 단색 농도만 쓴다 ([04 §2](../04-ui-spec.md#2-플래너-planner-탭), [05 §3](../05-localization-a11y.md#3-컬러-팔레트)).
...
- 절제된 톤. 채도를 높이지 않는다. **빨강 계열을 쓰지 않는다.**
...
- **히트맵에 빨강을 쓰고 싶어질 때**: 쓰지 않는다. 설계 원칙이다 ([05 §3](../05-localization-a11y.md#3-컬러-팔레트)).
